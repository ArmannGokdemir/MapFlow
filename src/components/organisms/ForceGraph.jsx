...
export default function ForceGraph({ graph, currentView, viewToggle, onNodeHover, onNodeOut, onNodeClick, onHoverCard }) {
  ...
  useEffect(() => {
    ...
    // Add zoom behavior with simpler, more reliable filtering
    const zoom = d3.zoom()
      .scaleExtent([0.2, 3])
      .filter((event) => {
        // Only allow zoom on wheel events
    ...
    // Set up force simulation with reduced collision detection
    const simulation = d3.forceSimulation(graph.nodes)
      .force("link", d3.forceLink(graph.links).id(d => d.id).distance(120))
      .force("charge", d3.forceManyBody().strength(-800))
      .force("center", d3.forceCenter(width / 2, height / 2))
      .force("collision", d3.forceCollide().radius(d => d.type === "process" ? 30 : 25).strength(0.7))
    ...
    // Create node groups - NO drag behavior initially
    ...
    nodeGroup.each(function(d) {
      ...
      if (d.type === 'process') {
        // Create a container for visual elements
        const visualContainer = group.append("g").attr("class", "visual-container");
        
        // Smaller circle for process nodes
        ...
        visualContainer.append("text")
          ...
          .attr("dy", "0.3em")
          .attr("class", "text-sm font-bold fill-white pointer-events-none select-none");
          
        // Add invisible larger circle for better dragging
        group.append("circle")
        ...
        if (processes.length > 1) {
          ...
          const pieData = createPieChart(d, 20);
          ...
          // Single process flow - smaller simple circle
    ...
    // Add drag behavior ONLY to drag handles
    ...
    // Apply drag behavior only to drag handles
    nodeGroup.selectAll(".drag-handle").call(dragBehavior);

    // Add hover effects ONLY to visual containers (not drag handles)
    nodeGroup.selectAll(".visual-container")
      .on("mouseenter", function(event, d) {
        // Don't show hover card if dragging
        ...
        // Longer debounce delay for hover card display
        ...
        // Immediate visual hover effects
        d3.select(this).selectAll(".node-visual")
          .transition()
          .duration(150)
          .style("filter", "brightness(1.1) drop-shadow(0 2px 4px rgba(0,0,0,0.15))")
          .style("transform", "scale(1.05)");
          
        // Make label slightly more prominent on hover
      ...
      .on("mousemove", function(event, d) {
        // Don't update hover card if dragging
        if (isDragging) return;
        
        // Only update position if hover card is already shown
        // And throttle updates significantly
        if (event.timeStamp - (this.lastMoveTime || 0) > 100) { // Increased from 50ms to 100ms
          // Check if there's an active hover card before updating position
